...
**_Edit:_** [TenetLang](https://twitter.com/tenetlang) now has _[a github repo](https://github.com/tenetlang/tenetlang), which I'll be
updating as I continue to work on it with GPT-4. Open a PR or submit a feature request!_
...
After my experience in the last 36h (I was up until 4am, recall), I believe this acceleration will only continue.
...
However, to my knowledge, none of these agents have ever been asked to make their _own_ programming language.<sup>1</sup> This type of task is a bit more complex, and requires a bit more creativity and foresight than much of the internet believes LLMs to have.

<sub>
  1. *EDIT: as pointed out by an HN poster, [some prior art does
  exist](https://judehunter.dev/blog/chatgpt-helped-me-design-a-brand-new-programming-language)*,
  though nothing yet for GPT-4 (that I can tell).
</sub>
<br />
<br />
Some folks on the internet tend to think lowly of LLMs as merely "Stochastic Parrots" -- simply "remixers"
of old ideas. In a way, they're right; but I'd argue that remixing is the fundamental force of creativity.
Nothing comes from scratch. > Nothing of me is original. I am the combined effort of everyone I've ever
known.

_― Chuck Palahniuk, Invisible Monsters_

<br />
Philosophy aside, I was curious -- **what would a being *made* of code, that has *read* most of the code
in the world, think about how to *design* code?**
...
<Thread>
  ...
  <GPT4Message>
    <MDXWrapper>
      In the updated example, `PersonService` represents a remote service definition, not a regular
      class. It serves as an interface for a remote data source that can fetch Person objects from
      another location or service, possibly running on a different machine or even implemented in a
      different programming language. By defining PersonService as an RPC service, we explicitly
      describe the contract for how clients can interact with this remote service.
      <br />
      In a distributed computing context, services are often spread across multiple nodes or
      processes to achieve better scalability, fault tolerance, and performance. The `PersonService`
      plays the role of a service that resides on a different node or process, and the main
      application communicates with it using a remote procedure call (RPC) mechanism.
      <br />
      In contrast, a class would typically represent an object within the same process or
      application, without built-in support for remote communication. By using an RPC service, we
      can abstract away the complexities of network communication, serialization, and
      deserialization, allowing developers to focus on the core functionality and logic of their
      application.
      <br />
      In the TenetLang example, we use the `rpc.connect` function to connect to a remote
      `PersonService` instance using a specified URL. This connection returns a "proxy" instance
      that can be used to make remote calls as if they were local method calls. The RPC mechanism
      transparently handles the network communication, serialization, and deserialization of data.
      <br />
      To summarize, the `PersonService` is defined as an RPC service instead of a class to emphasize
      its role as a remote service in a distributed computing scenario and facilitate communication
      between services in a more transparent and streamlined manner.
    </MDXWrapper>
  </GPT4Message>
</Thread>
<br />
...
<br />
*I only know about 10 programming languages with any level of decency, so some of these questions may
seem funny to the more well-read of you <sub>(ahem, HN).</sub>*
...
-- Luke
<br />
*PS: [TenetLang](https://github.com/tenetlang/tenetlang) now has a github repo, which I'll be updating
as I continue to work on it with GPT-4. Open a PR or submit a feature request!*
