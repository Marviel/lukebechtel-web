...
export default function Message({ name, avatarUrl, date, children, isOwn, compact }: MessageProps) {
    ...
    return (
        <Stack width={isMobile ? '95%' : '90%'} alignSelf={endOrStart}>
            <Card style={{ padding: extraPadding, backgroundColor: isOwn ? theme.palette.primary.main : theme.palette.grey[800], width: 'fit-content', minWidth: '100px', alignContent: endOrStart, alignItems: endOrStart, alignSelf: endOrStart }}>
                <Stack direction="row" justifyContent={'start'} gap={1} sx={{ width: '100%' }}>
                    {!isOwn && <Avatar alt={name} src={avatarUrl && avatarUrl !== '' ? avatarUrl : undefined} sx={{ width: avatarSize, height: avatarSize }} />}
                    <Stack>
